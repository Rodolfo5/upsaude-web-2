...
interface CreatePatientAuthRequest {
  email: string
  password: string
}

interface CreatePatientAuthResponse {
  uid?: string
  error?: string
}
...
export async function POST(
  request: Request,
): Promise<NextResponse<CreatePatientAuthResponse>> {
  try {
    const body: CreatePatientAuthRequest = await request.json()
